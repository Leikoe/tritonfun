...
def flash_attention_kernel(
    ...
    for j in tl.range(T_c):
        ...
        for i in tl.range(T_r):
            ...
            S_ij = tl.dot(Q_i, K_j.trans()) # (B_r, B_c)
            m_hat_ij = tl.max(S_ij, axis=1) # (B_r,)
            P_hat_ij = tl.exp(S_ij - m_hat_ij) # (B_r, B_c)
            ...
            _O_new_i_1 = l_i[:, None] * (tl.exp(m_i - m_new_i)[:, None] * O_i)
            print(_O_new_i_1.shape)
            _O_new_i_2 = tl.exp(m_hat_ij - m_new_i)[:, None] * P_hat_ij
...
def test_attention(B, H, N, D, atol=1e-3, rtol=1e-3, device="cuda:0"):
    ...
    scale = .1/math.sqrt(D)
    torch.testing.assert_close(flash_attention(q, k, v, scale=scale), F.scaled_dot_product_attention(q, k, v, scale=scale, is_causal=True), atol=atol, rtol=rtol)
...
@triton.testing.perf_report(
    triton.testing.Benchmark(
        ...
        line_arg="provider",
        line_vals=["torch_sdpa", "torch_naive"],
        line_names=["Torch sdpa", "Torch naive"],
...
def benchmark(N: int, provider: Literal["torch_sdpa"] | Literal["triton"]):
    ...
    if provider == "torch_sdpa":
        return triton.testing.do_bench(lambda: F.scaled_dot_product_attention(q, k, v, scale=scale, is_causal=True))
    if provider == "triton":
        return triton.testing.do_bench(lambda: flash_attention(q, k, v, scale=scale))

if __name__ == "__main__":
    test_attention(1, 1, 32, 64)
    # test_attention(1, 2, 1024, 128)
